 ...
 * );
 * 
 * -- Enable RLS and add public policies for ALL (SELECT, INSERT, DELETE, UPDATE)
 * ALTER TABLE hero_settings ENABLE ROW LEVEL SECURITY;
 * CREATE POLICY "Public access" ON hero_settings FOR ALL USING (true) WITH CHECK (true);
 * 
 * ALTER TABLE art_gallery ENABLE ROW LEVEL SECURITY;
 * CREATE POLICY "Public access" ON art_gallery FOR ALL USING (true) WITH CHECK (true);
 */
...
const App: React.FC = () => {
  ...
  // Store objects to track IDs for reliable deletion
  const [heroImages, setHeroImages] = useState<HeroImage[]>([]);
  const [artGallery, setArtGallery] = useState<ArtWork[]>([]);
  
  // Gallery upload state
  const [selectedArtBase64, setSelectedArtBase64] = useState<string | null>(null);
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      const { data: heros } = await supabase.from('hero_settings').select('*');
      if (heros && heros.length > 0) {
        setHeroImages(heros);
      } else {
        // Fallback
        setHeroImages([{ id: 0, url: "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=600" }]);
      }

      const { data: arts } = await supabase.from('art_gallery').select('*').order('id', { ascending: false });
      if (arts) setArtGallery(arts);
    } catch (err) {
  ...
  const addHeroImage = async (url: string) => {
    ...
    // Fixed: Returning data to get the generated ID
    const { data, error } = await supabase.from('hero_settings').insert([{ url }]).select();
    if (!error && data) {
      setHeroImages(prev => [...prev, data[0]]);
  ...
  const removeHeroImage = async (id: number) => {
    ...
    // Fixed: Deleting by primary key ID instead of URL string
  ...
  const handleArtUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => setSelectedArtBase64(reader.result as string);
      reader.readAsDataURL(file);
    }
  };

  const addArt = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedArtBase64) {
      alert("Please choose an image first.");
      return;
    }
    const title = artTitleRef.current?.value || '';
    const desc = artDescRef.current?.value || '';
    
    const { error } = await supabase.from('art_gallery').insert([{ url: selectedArtBase64, title, description: desc, comments: [] }]);
    if (!error) {
      fetchInitialData();
      setSelectedArtBase64(null);
  ...
  const updateArtComments = async (artId: string, updatedComments: Comment[]) => {
    const { error } = await supabase.from('art_gallery').update({ comments: updatedComments }).eq('id', artId);
    if (!error) setArtGallery(prev => prev.map(a => a.id === artId ? { ...a, comments: updatedComments } : a));
  };
  ...
  const identityItems: IdentityItem[] = [
    ...
    // Only show Direct Access if phone number is present
    ...(PERSONAL_INFO.phone ? [{ label: 'Direct Access', value: PERSONAL_INFO.phone, icon: <Phone size={20} className="text-green-500" /> }] : []),
              ...
              <motion.div key="settings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                  <GlassCard className="space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {heroImages.map((hero, i) => (
                        <div key={hero.id} className="relative aspect-[4/5] rounded-xl overflow-hidden group border border-slate-100 shadow-inner">
                           <img src={hero.url} className="w-full h-full object-cover" />
                           <button 
                    ...
                    <div className="space-y-4">
                      <input type="file" ref={fileInputRef} onChange={handleHeroUpload} accept="image/*" className="hidden" />
                      <button onClick={() => fileInputRef.current?.click()} className="w-full glass border-2 border-dashed border-purple-200 py-4 rounded-2xl font-bold tracking-widest text-purple-600 flex items-center justify-center gap-2 hover:bg-purple-50 transition-all">
                        <Upload size={18} /> UPLOAD NEW HERO IMAGE
                      </button>
                    </div>
                  </GlassCard>
                  ...
                  <GlassCard className="space-y-6">
                    ...
                    <form className="space-y-4" onSubmit={addArt}>
                      <div className="grid grid-cols-1 gap-4">
                        <input ref={artTitleRef} name="arttitle" required placeholder="Artwork Title" className="glass bg-white/50 border border-slate-200 p-4 rounded-2xl outline-none focus:border-orange-400" />
                        <textarea ref={artDescRef} name="artdesc" placeholder="Art description..." className="w-full glass bg-white/50 border border-slate-200 p-4 rounded-2xl outline-none h-24 resize-none focus:border-orange-400" />
                      </div>
                      
                      <div className="flex flex-col gap-3">
                        <input type="file" ref={artFileInputRef} className="hidden" accept="image/*" onChange={handleArtUpload} />
                        <button type="button" onClick={() => artFileInputRef.current?.click()} className={`w-full py-4 rounded-2xl font-bold text-xs tracking-widest border-2 border-dashed transition-all flex items-center justify-center gap-2 ${selectedArtBase64 ? 'border-orange-500 text-orange-600 bg-orange-50' : 'border-slate-200 text-slate-400 hover:bg-slate-50'}`}>
                           {selectedArtBase64 ? <CheckCircle size={18} /> : <ImageIcon size={18} />}
                           {selectedArtBase64 ? 'IMAGE SELECTED' : 'CHOOSE GALLERY IMAGE'}
                        </button>
                        <button type="submit" className="w-full bg-slate-900 text-white py-4 rounded-2xl font-bold tracking-widest hover:bg-orange-500 transition-all flex items-center justify-center gap-2 disabled:opacity-50" disabled={!selectedArtBase64}>
  ...
  return (
    <div className="min-h-screen relative text-slate-800 selection:bg-purple-200">
            ...
            <div className="relative glass p-2 lg:p-5 rounded-[24px] lg:rounded-[48px] shadow-2xl overflow-hidden group border border-white/50">
              <div className="overflow-hidden rounded-[20px] lg:rounded-[40px] bg-slate-100 aspect-[4/5] relative">
                {isInitialLoading ? (
                  ...
                  <AnimatePresence mode="wait">
                    <motion.img key={currentHeroIndex} initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ duration: 1 }} src={heroImages[currentHeroIndex]?.url} alt="Maisha Zaman" className="absolute inset-0 w-full h-full object-cover" />
                  </AnimatePresence>
                )}
                {heroImages.length > 1 && (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-6">
          ...
          <motion.div initial={{ scale: 0.9, y: 50 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 50 }} className="relative glass p-10 rounded-[48px] max-w-lg w-full shadow-3xl border border-white/60">
             ...
             <div className="space-y-6">
                {PERSONAL_INFO.phone && (
                  <>
                    <a href={`tel:${PERSONAL_INFO.phone}`} className="flex items-center justify-between p-6 bg-green-500 text-white rounded-3xl shadow-xl hover:bg-green-600 transition-all group">
                      <div className="flex items-center gap-4">
                        <div className="bg-white/20 p-3 rounded-2xl"><Phone size={24} /></div>
                        <div>
                          <p className="font-bold text-lg">Direct Call</p>
                          <p className="text-xs font-black uppercase tracking-widest">{PERSONAL_INFO.phone}</p>
                        </div>
                      </div>
                    ...
                    <div className="relative py-4">
                      ...
                      <div className="relative flex justify-center text-xs uppercase font-black text-slate-400 tracking-[0.3em]">
                        <span className="bg-[#fdfaf7] px-4">OR</span>
                      </div>
                    </div>
                  </>
                )}
      ...
      {PERSONAL_INFO.phone && (
        <a href={`https://wa.me/${PERSONAL_INFO.phone}`} target="_blank" className="fixed bottom-8 right-8 z-50 w-16 h-16 bg-green-500 text-white rounded-full shadow-2xl flex items-center justify-center transform hover:scale-110 active:scale-90 transition-all">
